...
contract Telopay is Ownable, ReentrancyGuard, Pausable {
        ...
        validETHAmount(msg.value) 
    {
        ...
        emit PaymentReceived(msg.sender, msg.value, true, paymentId, block.timestamp);
    }
        ...
        validUSDCAmount(amount) 
    {
        ...
        emit PaymentReceived(msg.sender, amount, false, paymentId, block.timestamp);
    }
     ...
     */
    function withdraw(uint256 ethAmount, uint256 usdcAmount) external onlyOwner nonReentrant {
        require(ethAmount <= address(this).balance, "Insufficient ETH balance");
        require(usdcAmount <= usdcToken.balanceOf(address(this)), "Insufficient USDC balance");
