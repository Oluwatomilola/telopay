...
contract TelopayTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(user1);
        usdcMock.approve(address(telopay), type(uint256).max);
    ...
    function testWithdrawAll_Success() public {
        ...
        vm.prank(owner);
        vm.expectEmit(true, true, true, true);
        emit Withdrawal(owner, expectedEthBalance, expectedUsdcBalance, block.timestamp);
        ...
        assertEq(usdcMock.balanceOf(owner), ownerInitialUsdcBalance + expectedUsdcBalance);
    }
    ...
    function testWithdraw_SpecificAmounts() public {
        ...
        assertEq(usdcMock.balanceOf(address(telopay)), MAX_USDC_PAYMENT - usdcToWithdraw);
        assertEq(owner.balance, ownerInitialEthBalance + ethToWithdraw);
        assertEq(usdcMock.balanceOf(owner), ownerInitialUsdcBalance + usdcToWithdraw);
    }
